...
impl CodeBlock {
    pub fn new(statement: Statement, collecting: bool) -> CodeBlock {
        CodeBlock { pipelines: Vec::new(), statement: statement, collecting: collecting }
    }
}
...
impl FlowControl {
    ...
    pub fn get_last_block(&self) -> &CodeBlock {
        self.blocks.last().expect("The blocks stack should not be empty")
    }

    pub fn get_last_block_mut(&mut self) -> &mut CodeBlock {
        self.blocks.last_mut().expect("The blocks stack should not be empty")
    }
    ...
    pub fn else_<I: IntoIterator>(&mut self, _: I) -> i32
        where I::Item: AsRef<str>
    {
        if let Statement::If(ref mut value) = self.get_last_block_mut().statement {
            *value = !*value;
            SUCCESS
        } else {
        ...
        where I::Item: AsRef<str>
    {
        ...
        if let Some(name) = args.nth(1) {
            let values: Vec<String> = args.map(|value| value.as_ref().to_string()).collect();
            self.blocks.push(CodeBlock::new(Statement::Function(name.as_ref().to_string(), values), true));
        } else {
